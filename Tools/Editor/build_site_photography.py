...
for folder in ('Content/Starbase/Materials','Content/Starbase/Meshes/Earth','Content/Starbase/Maps','Content/Starbase/Data'):
    ...
    for source in (root/folder).glob('*.umap'):
        ...
        if not destination.exists():
            destination.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(source,destination)
for script in ('import_earth_detail.py','build_world_surfaces.py','build_site_landscape_assets.py','finalize_site_coast.py','build_surface_materials.py','align_editor_sun.py'):
