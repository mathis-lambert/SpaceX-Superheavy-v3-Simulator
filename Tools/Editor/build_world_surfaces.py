...
ocean_slopes=import_texture('T_OceanSlopes',ART_ROOT/'Earth/WaterSpectrum/OceanSlopes.png',False,u.TextureCompressionSettings.TC_BC7)
prop(ocean_slopes,'address_x',u.TextureAddress.TA_WRAP);prop(ocean_slopes,'address_y',u.TextureAddress.TA_WRAP)
prop(ocean_slopes,'mip_gen_settings',u.TextureMipGenSettings.TMGS_SIMPLE_AVERAGE)
A.save_loaded_asset(ocean_slopes,False)
...
return lerp(C,float3(.52,.57,.56),foam*.5);''')
    ...
    waveuv0=custom(m,{'P':p,'T':time},'return P.xy/9600+float2(-.012,-.004)*T;',u.CustomMaterialOutputType.CMOT_FLOAT2)
    waveuv1=custom(m,{'P':p,'T':time},'return float2(dot(P.xy,float2(.932,.362)),dot(P.xy,float2(-.362,.932)))/15130+float2(.005,-.006)*T;',u.CustomMaterialOutputType.CMOT_FLOAT2)
    slope0=sample(m,ocean_slopes,waveuv0,sampler=u.MaterialSamplerType.SAMPLERTYPE_LINEAR_COLOR)
    slope1=sample(m,ocean_slopes,waveuv1,sampler=u.MaterialSamplerType.SAMPLERTYPE_LINEAR_COLOR)
    normal=custom(m,{'P':p,'Camera':camera,'T':time,'W':water,'Terrain':vertex,'Ground':(ground,'RGB'),'S0':slope0,'S1':slope1},normal_code())
