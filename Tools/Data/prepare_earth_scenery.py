...
tiles={(i,j):np.asarray(Image.open(ROOT/f'BocaChica_{i}_{j}.png').convert('RGBA')) for i in range(4) for j in range(4)}
...
for k in range(110000):
    ...
    px=int(((x+6000)/3000-i)*2047);py=int((1-((y+6000)/3000-j))*2047)
    r,g,b,a=map(float,tiles[i,j][py,px]);z=height(x,y)
