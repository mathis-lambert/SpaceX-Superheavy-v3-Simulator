param([int[]]$Cadences=@(60,30,15),[string]$EngineRoot='D:/Engines/UE_5.8',[string]$Prefix='Physical')
...
foreach ($fps in $Cadences) {
    foreach ($scenario in @('Nominal','Crosswind','Offset')) {
        ...
        $pass=$engineExit -eq 0 -and $cadence.measured -and $r.success -and $r.physical_capture -and $r.left_rail_contact -and $r.right_rail_contact -and $r.contact_engine_shutdown -and !$r.unpowered_thrust_violation -and $r.launch_catch_axis_distance_m -lt .01
