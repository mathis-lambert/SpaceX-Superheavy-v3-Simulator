...
foreach($view in $Views){
    ...
    @{view=$view;camera_altitude_m=$height;pitch_deg=$pitch;hour=$Hour;weather=$Weather;cloud_mode=$CloudMode;upsampling=$Upsampling;clouds_enabled=(!$NoClouds);cloud_shadows=(!$NoCloudShadows);reconstruction=$Reconstruction;output=@(2560,1440);warmup_seconds=15;capture_frames=360} | ConvertTo-Json | Set-Content -LiteralPath "$saved/$name.capture.json"
