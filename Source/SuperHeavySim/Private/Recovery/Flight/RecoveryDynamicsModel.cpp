...
FVector FRecoveryDynamicsModel::AttitudeMoment(const FRecoveryDynamicsCommand& C,double Gain,double Damping) const
{
    ...
    const FVector DesiredRate=(Axis*Rate+C.TargetAngularVelocityWorldRadS).GetClampedToMaxSize(RecoveryActuators::MaximumBodyRateRadS);
    const FVector Alpha=(DesiredRate-State.Body.AngularVelocityWorldRadS)*(Damping*C.Experiment.AttitudeResponse);
