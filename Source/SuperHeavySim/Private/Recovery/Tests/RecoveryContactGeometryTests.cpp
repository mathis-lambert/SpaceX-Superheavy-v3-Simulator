...
bool FRecoveryRailFootprintTest::RunTest(const FString&)
{
    ...
    TestFalse(TEXT("The front rail end cannot support the whole fitting"),RecoveryContactGeometry::OnUsableRailSpan(9));
    TestFalse(TEXT("The rear rail end cannot support the whole fitting"),RecoveryContactGeometry::OnUsableRailSpan(-17));
    TestFalse(TEXT("The end margin excludes near-edge contact"),RecoveryContactGeometry::OnUsableRailSpan(8));
